...
irc.Client = function (nick, server) {
  ...
  var User = function (nick) {
    return irc.extend(new irc.EventEmitter(), {
      ...
      /** Indicates whether the user is away or not. */
      away: false,
  ...
  var findUser = function (criteria) {
    return users.find(function (user) {
      return user.nick === criteria.trim() ||
        user.ident === criteria.trim();
    });
  };

  /** Finds a user by its nickname or creates it if it doesn't exist in the
   * user list.
   * @param {String} nickname User nickname. Cannot be null or empty.
   * @return {User} Returns the required user, never null.
   * @private
   * @methodOf irc.Client#
   */
  var findOrCreateUser = function (nickname) {
  ...
  var defaultErrorHandler = function (message) {
    ...
    error.target = target || targetName;
  ...
  var Commands = {
     ...
     * Implemented replies and errors (100%):
     *    - ERR_INVITEONLYCHAN
     *    - ERR_CHANNELISFULL
     *    - ERR_NOSUCHCHANNEL
     *    - ERR_TOOMANYTARGETS
    ...
    JOIN: {
      ...
      handler: function (message) {
        ...
        channel.emit("join", findOrCreateUser(getUserName(message.prefix)));
      }
    },
    ...
    PART: {
      id: "part",
      handler: function (message) {
        var user = findOrCreateUser(getUserName(message.prefix));
     ...
     * Implemented replies and errors:
    ...
    /** Notice to either a user or a channel. Same as PRIVMSG, take a look at
     * documentation: http://tools.ietf.org/html/rfc2812#section-3.3.2
     */
    NOTICE: {
      id: "notice",
      handler: function (message) {
        var targetName = message.params[0].trim();
        var target = channels[targetName] || findUser(targetName);
        var notice = message.params[1];

        if (target) {
          target.emit("notice", notice);
        }
        client.emit("notice", {
          target: targetName,
          text: notice
    ...
    /** Channel and user mode messages. Partially implemented.
     * User message: http://tools.ietf.org/html/rfc2812#section-3.1.5
     * Channel message: http://tools.ietf.org/html/rfc2812#section-3.2.3
     */
    MODE: {
      id: "mode",
      handler: function (message) {
        var targetName = message.params[0].trim();
        var target = channels[targetName] || findUser(targetName);
        var modes = message.params[1];

        if (target) {
          target.modes = modes;
          target.emit("info", "modes", modes);
        }
        client.emit("mode", {
          target: targetName,
          modes: modes
    ...
    RPL_WHOISUSER: {
      id: 311,
      handler: function (message) {
        var user = findOrCreateUser(message.params[1]);
    ...
    /** WHOIS server information. Part of WHOIS reply.
     * http://tools.ietf.org/html/rfc2812#section-3.6.2
     */
    RPL_WHOISSERVER: {
      id: 312,
      handler: function (message) {
        ...
        user.server = {
          host: message.params[2],
          info: message.params[3]
        };
      }
    },
    ...
    RPL_AWAY: {
      ...
      handler: function (message) {
        ...
        if (user) {
          user.away = true;
          user.emit("away", message.params[2]);
        }
      }
    },
    ...
    RPL_TOPIC: {
      id: 332,
      handler: function (message) {
        var channelName = message.params[1].trim();
        var channel = channels[channelName];
        var topic = message.params[2];

        if (channel) {
          channel.topic = topic;
          channel.emit("info", "topic", topic);
        }
        client.emit("topic", {
  ...
  return irc.extend(client, new irc.EventEmitter(), {
     ...
     */
    user: function (nick) {
      var user = findOrCreateUser(nick.trim());
